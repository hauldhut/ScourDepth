...
# datafiles.Com = c("Culvert.csv")#labels=c("A", "B")
datafiles.Com = c("Sluice.csv")#labels=c("C", "D")
...
Methods = c("GMDHreg_G1","GMDHreg_G2")
Methods.Short = c("GMDH (degree=1)","GMDH (degree=2)")
...
Methods.Com = c("GMDHreg_G1","GMDHreg_G2")
...
library('cowplot')
plot_grid(h.Res2Plot[["RMSE"]], h.Res2Plot[["Cor"]], labels=c("C", "D"), ncol = 2, nrow = 1)

summaryfile = paste0(FigureType,"_",paste0(Methods.Short,collapse = "-"),"_",paste0(datafiles.Com,collapse = "-"))
print(summaryfile)
ggsave(paste0(summaryfile,".pdf"), width = 10, height = 5)
